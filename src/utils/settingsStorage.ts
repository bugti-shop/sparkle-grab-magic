...
// Timeout wrapper to prevent IndexedDB from hanging forever on web
...
// In-memory warm cache — populated by warmSettingsCache / getAllSettings
const _warmCache = new Map<string, any>();
let _warmCacheReady = false;

/** Pre-load ALL settings into memory in a single IndexedDB read.
 *  Call once at startup so subsequent getSetting calls are synchronous. */
export const warmSettingsCache = async (): Promise<void> => {
  if (_warmCacheReady) return;
  try {
    const all = await getAllSettings();
    for (const [k, v] of Object.entries(all)) {
      _warmCache.set(k, v);
    }
    _warmCacheReady = true;
  } catch {
    // Non-fatal — getSetting will fall back to IndexedDB
  }
};

// Core get/set functions
export const getSetting = async <T>(key: string, defaultValue: T): Promise<T> => {
  // Fast path: return from warm cache if available
  if (_warmCacheReady && _warmCache.has(key)) {
    return _warmCache.get(key) as T;
  }
  ...
  try {
    return await withRetry((database) => new Promise<T>((resolve) => {
      ...
      request.onsuccess = () => {
        const val = request.result !== undefined ? (request.result.value as T) : defaultValue;
        _warmCache.set(key, val);
        resolve(val);
      };
