...
export const getPriorities = async (): Promise<CustomPriority[]> => {
  ...
  if (saved && saved.length > 0) {
    // Ensure default priorities get updated colors when defaults change
    const defaultMap = new Map(DEFAULT_PRIORITIES.map(p => [p.id, p]));
    return saved.map(p => {
      const def = defaultMap.get(p.id);
      // If it's a default priority and still has an old default color, update it
      if (def && p.isDefault && p.color !== def.color) {
        return { ...p, color: def.color };
      }
      return p;
    });
  }
