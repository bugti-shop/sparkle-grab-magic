...
const CERT_META: Record<string, { title: string; subtitle: string; accent: string; icon: React.ReactNode }> = {
  first_step: { title: 'First Step', subtitle: 'Welcome to Flowist', accent: 'hsl(160, 70%, 50%)', icon: React.createElement(Shield, { className: 'h-5 w-5' }) },
  ...
  useEffect(() => {
    const handler = async () => {
      ...
      try {
        ...
        const [tasks, notes, folders, seenCerts, streakRaw, adminBypass, firstStepData] = await Promise.all([
          loadTodoItems(),
          loadNotesFromDB(),
          loadFolders(),
        ...
        const CERT_REQS: Record<string, { tasksCompleted: number; streakDays: number; notesCreated: number; foldersUsed: number }> = {
          first_step: { tasksCompleted: 0, streakDays: 0, notesCreated: 0, foldersUsed: 0 },
        ...
        const isUnlocked = (certId: string) => {
          if (certId === 'first_step') return !!firstStepData;
